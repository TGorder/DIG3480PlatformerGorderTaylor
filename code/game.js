...
function scoreBoard() {
  ...
  var cAndDnode = document.createTextNode("|");
  para.appendChild(cAndDnode);
}

function scoreBoardDeath() {
  var para = document.getElementById("p1");
  var cAndDnode = document.createTextNode("-");
  para.appendChild(cAndDnode);
}
...
Player.prototype.size = new Vector(0.6, 1.8);
Player.prototype.type = "player";

Player.prototype.changeSize = function() {
  this.size = new Vector(1.2, 3.6);
  //changePos(this);
  Player.prototype.imBig = 1;
}
...
// size and position
function Jello(pos, ch) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, 0.1));
  this.size = new Vector(1, 1);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Jello.prototype.type = "jello";

function Breakaway(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0));
  this.size = new Vector(1,1);
}
Breakaway.prototype.type = "breakaway";
...
Jello.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};

Breakaway.prototype.act = function(step, level) {
...
var playerXSpeed = .5;
var playerMaxXSpeed = 8;

Player.prototype.moveX = function(step, level, keys) {
  if (keys.left) {
    this.speed.x -= playerXSpeed;
    this.speed.x = -Math.min(-this.speed.x, playerMaxXSpeed);
  } else if (keys.right) {
    this.speed.x += playerXSpeed;
    this.speed.x = Math.min(this.speed.x, playerMaxXSpeed);
  } else if (this.speed.x > 0.5 || this.speed.x < -0.5) {
    this.speed.x = (this.speed.x - (this.speed.x / (Math.abs(this.speed.x)) * .3));
  } else if (this.speed.x != 0) {
    this.speed.x = Math.abs(this.speed.x) - Math.abs(this.speed.x)
  }
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.actors = this.actors.filter(function(other) {
    //call Player constructor function
    return other != actor;
    });
    ...
    if (playerMaxXSpeed > 0) {
      playerMaxXSpeed = 2;
    } else if (playerMaxXSpeed < 0) {
      playerMaxXSpeed = -2;
    }
  } else {
    playerMaxXSpeed = 8;
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        alert("Congratulations! You've escaped the lab! Be free jello man, be free.");
    });
  }
  startLevel(0);
}

alert("Escape, Jello man! Be free!")
